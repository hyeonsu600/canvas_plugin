...
describe('ClosedCaptionCreatorRow', () => {
    ...
    describe('when showing configured caption data', () => {
        it('renders normally', () => {
            const { getByText } = render(_jsx(ClosedCaptionCreatorRow, { ...makeConfiguredProps() }));
            expect(getByText('English')).toBeInTheDocument();
            expect(getByText('Remove English closed captions')).toBeInTheDocument();
        });
        it('renders tooltip for inherited captions', () => {
            const { getByRole } = render(_jsx(ClosedCaptionCreatorRow, { ...makeConfiguredProps(), inheritedCaption: true }));
        ...
        it('does not renders tooltip for non-inherited captions', () => {
            const { queryByRole } = render(_jsx(ClosedCaptionCreatorRow, { ...makeConfiguredProps(), inheritedCaption: false }));
            expect(queryByRole('tooltip', {
                name: 'Captions inherited from a parent course cannot be removed. You can replace by uploading a new caption file.',
            })).not.toBeInTheDocument();
        });
        it('calls onDeleteRow when trashcan is clicked', () => {
            const onDeleteRow = vi.fn();
            const { getByText } = render(_jsx(ClosedCaptionCreatorRow, { ...makeConfiguredProps({ onDeleteRow }) }));
    ...
    describe('when editing caption data', () => {
        const selectFile = async (element, file) => {
            await waitFor(() => fireEvent.change(element, {
                target: {
                    files: [file],
                },
            }));
        ...
        it('renders normally', () => {
            const { getByText } = render(_jsx(ClosedCaptionCreatorRow, { ...makeProps() }));
        ...
        it('renders selected file name if a file is selected', () => {
            const { getByText } = render(_jsx(ClosedCaptionCreatorRow, { ...makeProps({ selectedFile: { name: 'caps.srt' } }) }));
            expect(getByText('caps.srt')).toBeInTheDocument();
        });
        it('renders selected language when a language is selected', () => {
            const { getByDisplayValue } = render(_jsx(ClosedCaptionCreatorRow, { ...makeProps({ selectedLanguage: { id: 'fr', label: 'French' } }) }));
            expect(getByDisplayValue('French')).toBeInTheDocument();
        });
        it('calls onFileSelected when file is selected', async () => {
            const onFileSelected = vi.fn();
            const { container } = render(_jsx(ClosedCaptionCreatorRow, { ...makeProps({
                    onFileSelected,
                }) }));
        ...
        it('upload a file that size is bigger than permitted', async () => {
            const onFileSelected = vi.fn();
            const { container, getAllByText } = render(_jsx(ClosedCaptionCreatorRow, { ...makeProps({
                    onFileSelected,
                    userLocale: 'en',
                }) }));
            const fileInput = container.querySelector('input[type="file"]');
