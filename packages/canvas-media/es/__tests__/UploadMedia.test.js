...
function createPanel(overrideProps, ref) {
    return (_jsx(ComputerPanel, { ref: ref, theFile: null, setFile: () => { }, hasUploadedFile: false, setHasUploadedFile: () => { }, label: "Upload File", uploadMediaTranslations: uploadMediaTranslations, accept: ACCEPTED_FILE_TYPES, userLocale: "en", liveRegion: () => document.getElementById(LIVE_REGION_ID), updateSubtitles: () => false, useStudioPlayer: false, ...overrideProps }));
}
...
describe('UploadMedia: ComputerPanel', () => {
    ...
    it('shows an editable text input for title if a valid file is added', async () => {
        ...
        const setFile = (file) => rerender(createPanel({
            setFile,
            theFile: file,
    ...
    describe('file preview', () => {
        beforeEach(() => {
            vi.mock('@instructure/studio-player', () => ({
                StudioPlayer: vi.fn(() => _jsx("div", { "data-testid": "studio-player" })),
            }));
            // Mock URL object
    ...
    describe('shows closed captions panel', () => {
        ...
        beforeEach(() => {
            ...
            vi.mock('../ClosedCaptionCreator', () => ({
                default: () => _jsx("div", { "data-testid": "ClosedCaptionPanel" }),
            }));
        });
        afterEach(() => {
            vi.clearAllMocks();
