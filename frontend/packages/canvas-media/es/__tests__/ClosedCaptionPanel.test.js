...
describe('ClosedCaptionCreator', () => {
    ...
    describe('default export', () => {
        it('loads translations', async () => {
            render(_jsx(ClosedCaptionCreator, { ...makeProps({ userLocale: 'es' }) }));
            await waitFor(() => expect(getTranslations).toHaveBeenCalledWith('es'));
        });
    });
    it('renders normally', () => {
        const { getByTestId } = render(_jsx(ClosedCaptionPanel, { ...makeProps() }));
        expect(getByTestId('CC-CreatorRow-choosing')).toBeInTheDocument();
    });
    it('selects a file', () => {
        const updateSubtitles = vi.fn();
        const { container, getByText, getByPlaceholderText, getByTestId } = render(_jsx(ClosedCaptionPanel, { ...makeProps({ updateSubtitles }) }));
        ...
        expect(updateSubtitles).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ locale: 'fr' })]));
    });
    it('adds a new row and focused language selector when + is clicked', () => {
        const { container, getByText, getByPlaceholderText, getAllByTestId } = render(_jsx(ClosedCaptionPanel, { ...makeProps() }));
    ...
    it('includes inherited caption languages in the list to upload captions', () => {
        const { getByText, getByRole, getByPlaceholderText } = render(_jsx(ClosedCaptionPanel, { ...makeProps({
                subtitles: [
    ...
    describe('focus', () => {
        it('moves to next CC on deleting one', () => {
            const { container, getByText, getAllByTestId } = render(_jsx(ClosedCaptionPanel, { ...makeProps({
                    subtitles: [
            ...
            fireEvent.click(trashcan);
            expect(getAllByTestId('CC-CreatorRow-chosen')).toHaveLength(2);
            expect(getByText('Remove Spanish closed captions').closest('button')).toBe(container.ownerDocument.activeElement);
        });
        it('moves to add button on deleting last current one', () => {
            ...
            fireEvent.click(trashcan);
            expect(getAllByTestId('CC-CreatorRow-chosen')).toHaveLength(2);
            expect(getByText('add new caption').closest('button')).toBe(container.ownerDocument.activeElement);
        });
        it('moves to language select on clicking the add button', () => {
            const { container, getByText, getByPlaceholderText } = render(_jsx(ClosedCaptionPanel, { ...makeProps({
                    subtitles: [
                        { locale: 'en', file: { name: 'en.srt' } },
                        { locale: 'es', file: { name: 'es.srt' } },
                    ],
