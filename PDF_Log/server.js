...
const defaultConfig = {
  development: {
    ...
    TOOL_URL: 'https://canvas.docker:3003'
  },
  production: {
    CANVAS_LMS_URL: 'https://lms.kimhaksa.com',
    TOOL_URL: 'https://test2.kimhaksa.com'
  }
};
...
console.log(`🔧 PDF_Log 환경변수 설정 완료 (${isDev ? '개발' : '프로덕션'} 모드):`, {
...
app.prepare().then(() => {
  ...
  server.listen(port, err => {
    if (err) throw err;
    console.log(`✅ PDF_Log HTTP server ready on http://localhost:${port}`);
    console.log(`🔒 HTTPS는 Nginx Proxy Manager가 처리합니다.`);
    console.log(`🎯 PDF_Log LTI Client ID: ${process.env.LTI_CLIENT_ID}`);
  });
}); 